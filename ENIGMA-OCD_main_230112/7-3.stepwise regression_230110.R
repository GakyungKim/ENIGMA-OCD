...
setwd("~/Desktop/study_enigma-ocd/0.Data/Dai result/After piras updated_Adult/T.Dx_D.nonharmo scaled only adult_cv.LOSO_777_F.age.sex_21.11.23")
...
#### add AO_binary #####
train_pred_OCD %<>% mutate(AO_binary = ifelse(Dx == 1 & AO < 18, 'childhood-onset', 
...
test_pred_OCD %<>% mutate(AO_binary = ifelse(Dx == 1 & AO < 18, 'childhood-onset',
                                             ifelse(Dx == 1 & AO >= 18, 'adult-onset', NA)), 
                          AO_binary = as.factor(AO_binary), 
                          AO_child = ifelse(AO_binary == 'childhood-onset', 1, 
                                            ifelse(AO_binary == 'adult-onset', 0, NA)), 
...
train_pred_OCD %>% xtabs(~AO_binary, data = .)
...
test_pred_OCD %>% xtabs(~AO_binary, data = .)
...
library(MASS)
train_pred_OCD_step <- train_pred_OCD[,colnames(train_pred_OCD)[c(3, 5, 7, 8:20, 274, 276)]] #  552 × 18
train_pred_OCD_step_na <- na.omit(train_pred_OCD_step) %>% as_tibble()#  379 × 18
model.train = lm(Dx.1 ~ 1, data = train_pred_OCD_step_na)
step.forward_train = stepAIC(model.train, 
                             direction = "forward", 
                             scope = Dx.1 ~ Med + AO_child + Dur + Sev + Agr_Check + Clean + Sex_Rel 
                             + Hoard + Ord + Anx + CurAnx + Dep + CurDep + Age + Sex + Site)

step.forward_train

############# final model in validation sample 
model.final_train <- lm(Dx.1 ~ Site + Age + Hoard + AO_child + CurDep , data = train_pred_OCD_step_na)
summary(model.final_train)


aov.final_train <- train_pred_OCD_step_na %>% rstatix::anova_test(Dx.1 ~ Site + Age + Hoard + AO_child + CurDep)
rstatix::get_anova_table(aov.final_train)

#################### 4.2.2. test set ####################
##### preprocessing
test_pred_OCD_step <-  test_pred_OCD[,colnames(test_pred_OCD)[c(3, 5, 7, 8:20, 274, 276)]] %>% as_tibble() # 138 x 17
test_pred_OCD_step_na <- na.omit(test_pred_OCD_step) # 104 x 17


################### 1) stepwise regression model identified in train set
model.steptrain_test <- lm(Dx.1 ~ Site + Age + Hoard + AO_child + CurDep, data = test_pred_OCD_step_na)
summary(model.steptrain_test)

aov.steptrain_test <- test_pred_OCD_step_na %>% rstatix::anova_test(Dx.1~ Site + Age+ Hoard + AO_child + CurDep)
...
##########################################################################
###### load prediction probability data
setwd("~/Desktop/study_enigma-ocd/0.Data/Dai result/After piras updated_Pediatric/Pedi_dx_scaled")
...
pedi_step.forward_train = stepAIC(pedi_model.train, 
                                  direction = "forward", 
                                  scope = Dx.1 ~ Med + Dur + Sev + Agr_Check + Clean + Sex_Rel 
                                  + Hoard + Ord + Anx + CurAnx + Dep + CurDep + Age + Sex + Site)
